...
import * as gitLogParser from './git-parser/git-log-parser.js'
 ...
 */
export function getCommits(from, to, execaOptions) {
  const commits = [];
  return new Promise((resolve,reject)=>{
    gitLogParser.parse(
      ...
      commits.push({ ...commit, message: commit.message.trim(), gitTags: commit.gitTags.trim() });
    }).on('end',()=>{
      resolve(commits);
    }).on('error',(error)=>{
      reject(error);
...
/**
 * Get gits root directory.
 *
 * @param {Object} [execaOpts] Options to pass to `execa`.
 */
export async function getGitDirectory(execaOptions) {
  const result = await execa("git", ["rev-parse", "--show-toplevel" ], execaOptions);
